...
module.exports = function IndexModule(pb) {
    ...
    /**
     * Index The home page controller of the probo theme.
     * @class Index
    ...
    Index.prototype.render = function(cb) {
        ...
        TopMenu.getTopMenu(self.session, self.ls, options, function(themeSettings, navigation, accountButtons) {
            TopMenu.getBootstrapNav(navigation, accountButtons, function(navigation, accountButtons) {
                ...
                pluginService.getSettings('probo', function(err, proboSettings) {
                    var homePageKeywords = '';
                    var homePageDescription = '';
                    for(var i = 0; i < proboSettings.length; i++) {
                        switch(proboSettings[i].name) {
                            case 'home_page_keywords':
                                homePageKeywords = proboSettings[i].value;
                                break;
                            case 'home_page_description':
                                homePageDescription = proboSettings[i].value;
                                break;
                            default:
                                break;
                        }
                    ...
                    self.ts.load('landing_page', function(err, template) {
                        ...
                        var dao = new pb.DAO();
                        dao.q('probo_theme_settings', opts, function(err, settings) {
                            if (util.isError(err)) {
